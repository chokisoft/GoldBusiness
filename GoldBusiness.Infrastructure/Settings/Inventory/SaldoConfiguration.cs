...
namespace GoldBusiness.Infrastructure.Settings.Inventory
{
    public class SaldoConfiguration : IEntityTypeConfiguration<Saldo>
    {
        public void Configure(EntityTypeBuilder<Saldo> builder)
        {
            ...
            builder.HasOne(d => d.Localidad).WithMany(p => p.Saldos).HasForeignKey(d => d.LocalidadId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_Saldo_Localidad");
            builder.HasOne(d => d.Producto).WithMany(p => p.Saldos).HasForeignKey(d => d.ProductoId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_Saldo_Producto");
        }
    }
}
