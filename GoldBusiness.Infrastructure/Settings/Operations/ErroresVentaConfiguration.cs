...
namespace GoldBusiness.Infrastructure.Settings.Operations
{
    public class ErroresVentaConfiguration : IEntityTypeConfiguration<ErroresVenta>
    {
        public void Configure(EntityTypeBuilder<ErroresVenta> builder)
        {
            ...
            builder.HasOne(d => d.OperacionesDetalleNavigation).WithMany().HasForeignKey(d => d.OperacionesDetalleId).OnDelete(DeleteBehavior.Cascade).HasConstraintName("FK_ErroresVenta_OperacionesDetalle");
            builder.HasOne(d => d.LocalidadNavigation).WithMany(p => p.ErroresVenta).HasForeignKey(d => d.LocalidadId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_ErroresVenta_Localidad");
            builder.HasOne(d => d.ProductoNavigation).WithMany(p => p.ErroresVenta).HasForeignKey(d => d.ProductoId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_ErroresVenta_Producto");
        }
    }
}
