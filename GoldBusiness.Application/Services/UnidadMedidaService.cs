...
namespace GoldBusiness.Application.Services
{
    public class UnidadMedidaService : IUnidadMedidaService
    {
        ...
        public async Task<(IEnumerable<UnidadMedidaDTO> Items, int Total)> GetPagedAsync(int page, int pageSize, string? termino = null, string lang = "es")
        {
            var (items, total) = await _repo.GetPagedAsync(page, pageSize, termino);
            var dtos = items.Select(u => MapToDTO(u, lang))
                            .Where(dto => dto is not null)
                            .Select(dto => dto!)
                            .ToList();
            return (dtos, total);
